...
$isWhatsAppSettingsActive = $isActive(['/whatsapp-settings']);
$isConfigOpen = str_starts_with($currentPath, '/users') || str_starts_with($currentPath, '/roles') || $isMailSettingsActive || $isWhatsAppSettingsActive;
        ...
        <?php if ($hasConfigItems): ?>
            <details class="nav-group" <?= $isConfigOpen ? 'open' : '' ?>>
                ...
                <div class="nav-submenu">
                    ...
                    <?php if ($canConfigureAdmissions): ?><a href="<?= App::url('/whatsapp-settings') ?>" class="nav-sublink <?= $isWhatsAppSettingsActive ? 'is-active' : '' ?>">Plantilla WhatsApp</a><?php endif; ?>
                </div>
            </details>
        <?php endif; ?>
    </nav>
